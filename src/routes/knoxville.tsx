...
import { isNightlife, venuesIn } from "@/data/venues";
import {
  AREAS_BY_REGION,
  CATEGORIES,
  REGIONS,
...
export const Route = createFileRoute("/knoxville")({
  ...
  validateSearch: (raw: Record<string, unknown>): HomeSearch => {
    ...
    if (isRegion(raw.region) && raw.region !== "downtown") next.region = raw.region;
...
function hrefFor(next: {
  ...
  if (next.region && next.region !== "downtown") p.set("region", next.region);
  ...
  const s = p.toString();
  return s ? `/knoxville?${s}` : "/knoxville";
}

function Home() {
  const search = Route.useSearch();
  const region = search.region ?? "downtown";
  ...
  const districts = AREAS_BY_REGION[region];
  const area: Area | "All" = districts.includes(search.area as Area)
    ? (search.area as Area)
    : "All";
  const pool = useMemo(() => venuesIn(region), [region]);
  ...
  return (
    <div className="min-h-screen bg-bg pb-40 text-fg">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[10px] font-medium tracking-[0.22em] text-accent uppercase">
              Knox County field guide
            </p>
            <h1 className="font-display text-2xl leading-none sm:text-3xl">
              After Dark Knox
            </h1>
            <a href="/" className="mt-2 inline-block text-[11px] text-accent no-underline">
              Sunny Isles–South Beach guide →
            </a>
          </div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-3 sm:px-6">
        <form method="get" action="/knoxville" className="relative block">
      ...
      <main className="mx-auto max-w-6xl px-4 pb-8 sm:px-6">
        ...
        <footer className="mt-16 border-t border-line py-8 text-xs leading-relaxed text-faint">
          Scores describe typical Thursday–Saturday nights unless noted. Pick a region tab first — Downtown, West (through Farragut), North (Fountain City to Halls), South (Sevier Ave to Chapman), or East (Magnolia to Holston) — then sort that tab only. Regions do not mix. Estimates, not a census. Snapshot 2026.
        </footer>
      </main>
